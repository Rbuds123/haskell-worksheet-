...
describePlaceTemperatures placesTemperatures = [(place, describeTemperature temp) | (place, temp) <- placesTemperatures]
...
hotter _ [] = []
hotter temp ((place, placeTemp):rest)
    | placeTemp > temp = place : hotter temp rest
    | otherwise = hotter temp rest
-- *** Uncomment the line above and add your code here ***
...
insert x Null = Node x Null  -- If the list is empty, create a new node with the value x
insert x (Node val rest)
    | x <= val  = Node x (Node val rest)  -- If x is less than or equal to the current value, insert x before the current node
    | otherwise = Node val (insert x rest)  -- Otherwise, recurse on the rest of the list
...
main :: IO ()
main = do
    putStrLn "Testing price1 function:"
    putStrLn ("Price of Pizza: " ++ show (price1 "Pizza"))
    putStrLn ("Price of Pasta: " ++ show (price1 "Pasta"))
    putStrLn ("Price of Salad: " ++ show (price1 "Salad"))
    putStrLn ("Price of Burger: " ++ show (price1 "Burger"))